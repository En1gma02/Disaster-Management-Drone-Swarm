...
def get_facing_corners(geofence_polygon, uav_start_position, k=3):
    """
    Selects corners that are on the "visible face" of the polygon relative to the UAV,
    filtering out corners on the "back" side which would require flying around the shape.
    This effectively implements a "Field of View" filter before distance sorting.
    """
    # 1. Fallback if inside: standard closest corners
    if geofence_polygon.contains(uav_start_position):
        return get_k_closest_corners(geofence_polygon, uav_start_position, k)

    
    centroid = geofence_polygon.centroid
    # Vector: Centroid -> Drone
    v_drone = np.array([uav_start_position.x - centroid.x, uav_start_position.y - centroid.y])
    
    corners = [Point(c) for c in geofence_polygon.exterior.coords[:-1]]
    visible_candidates = []

    for pt in corners:
        # Vector: Centroid -> Corner
        v_corner = np.array([pt.x - centroid.x, pt.y - centroid.y])
        
        # Dot product >= 0 implies angle <= 90 degrees (facing the drone)
        if np.dot(v_drone, v_corner) >= -1e-9: # mild epsilon for float jitter
            visible_candidates.append(pt)
            
    # Rescue: If filter killed everything (rare/impossible for convex, possible for concave), use all
    if not visible_candidates:
        visible_candidates = corners

    # Sort candidates by simple distance
    candidates_sorted = sorted(visible_candidates, key=lambda p: uav_start_position.distance(p))
    
    unique = []
    seen = set()
    for pt in candidates_sorted:
...
def choose_best_entry_point(geofence_polygon, uav_start_position, overlap, sensor_footprint_width):
    ...
    """
    # Use the Visible Face logic to pick candidates
    candidates = get_facing_corners(geofence_polygon, uav_start_position, k=3)
    debug = []
    candidate_paths = []
    results = []
    ...
    return best_entry, best_path, debug, candidates, final_candidate_paths, best_index
